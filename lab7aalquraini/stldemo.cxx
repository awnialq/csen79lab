...
int main(int argc, char *argv[]) {
	...
	constexpr int target(174);	// what we are searching
	...
	if (lower_bound(vlist.begin(), vlist.end(), target, lessFunctor) != vlist.end())
		cout << "Found " << *it << " (" << it - vlist.begin() << ") >= " << target << endl;

	if (upper_bound(vlist.begin(), vlist.end(), target, lessFunctor) != vlist.end())
		cout << "Found " << *it << " (" << it - vlist.begin() << ") > " << target << endl;
